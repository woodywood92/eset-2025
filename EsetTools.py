...
class EsetRegister(object):
    ...
    def confirmAccount(self):
        ...
        uCE(self.driver, 'return document.title.indexOf("ESET") !== -1')
        console_log('\ndocument.title passed ', INFO)
...
class EsetKeygen(object):
    ...
    def sendRequestForKey(self):
        ...
        uCE(self.driver, f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'data-label', 'onboarding-welcome-skip-introduction-btn'))") # V2

        console_log('Onboarding ...', INFO)
        uCE(self.driver, f"return {CLICK_WITH_BOOL}({GET_EBAV}('label', 'data-label', 'onboarding-add-subscription-protect-card-trial'))")
        console_log('Click under trial 30 days !', OK)

        console_log('Click continue ...', INFO)
        uCE(self.driver, f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'aria-disabled', 'false'))")
        console_log('Continue ok !!!!', OK)

        console_log('Choose your home ...', INFO)
        uCE(self.driver, f"""var label = document.querySelector('label.css-1od4xk2'); return {CLICK_WITH_BOOL}(label);""")
        console_log('Click home !', OK)
        ...
        console_log('Click continue ...', INFO)
        uCE(self.driver, f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'aria-disabled', 'false'))")
        console_log('Step done - go to add member', OK)

        console_log('Adding member ...', INFO)
        uCE(self.driver, "return typeof document.getElementById('name') === 'object'")
        exec_js(f"""const nameField = {GET_EBAV}('input', '', 'name');if (nameField) {{nameField.value = 'Paul Dupont';nameField.dispatchEvent(new Event('input', {{ bubbles: true }}));}}""")
        uCE(self.driver, f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'aria-disabled', 'false'))")
        console_log('Member added ok !!', OK)

        console_log('1st member added ...', INFO)
        uCE(self.driver, f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'aria-disabled', 'false'))")
        console_log('Continue ok !', OK)

        console_log('Choose who would use this account ...', INFO)
        uCE(self.driver,f"""var firstOption = document.querySelector('div.css-9f1f65 label');return {CLICK_WITH_BOOL}(firstOption);""")
        console_log('Click me only !', OK)

        console_log('Click continue ...', INFO)
        uCE(self.driver, f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'aria-disabled', 'false'))")
        console_log('Continue ok !', OK)

        console_log('Protect other device ...', INFO)
        uCE(self.driver, f"return {CLICK_WITH_BOOL}({GET_EBAV}('button', 'aria-disabled', 'false'))")
        console_log('finish setup !', OK)
    ...
    def getLicenseData(self):
        ...
        if platform.release() == '7' and sys.platform.startswith('win'): # old browser versions
            ...
            uCE(self.driver, f"""var btn = document.querySelector('button.css-f58z5q');return {CLICK_WITH_BOOL}(btn);""")

        if self.driver.current_url.find('detail') != -1:
            console_log(f'License ID: {self.driver.current_url[-11:]}', OK)
        console_log('Je suis la !', OK)



        # récupérer le nom du produit
        license_name = exec_js(f"""
        var el = {GET_EBAV}('div', 'data-label', 'license-detail-product-name');
        return el ? (el.querySelector('p') ? el.querySelector('p').innerText : '') : '';
        """)
